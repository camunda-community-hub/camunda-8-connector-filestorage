...
@OutboundConnector(name = "fileStorageFunction", inputVariables = { FileStorageInput.INPUT_FOLDER,
    FileStorageInput.INPUT_FILE_NAME, FileStorageInput.INPUT_FILTER_FILE,
    FileStorageInput.INPUT_POLICY, FileStorageInput.INPUT_STORAGEDEFINITION, FileStorageInput.INPUT_STORAGEDEFINITION_FOLDER_COMPLEMENT,
...
public class FileStorageFunction implements OutboundConnectorFunction, CherryConnector {
  ...
  public static final List<Class<?>> allFunctions = Arrays.asList(UploadFile.class,
      DownloadFile.class, DeleteFile.class);
  ...
  public FileStorageOutput execute(OutboundConnectorContext outboundConnectorContext) throws Exception {
    FileStorageInput pdfInput = outboundConnectorContext.bindVariables(FileStorageInput.class);
    // search the sub-function referenced
    String function = pdfInput.getFileStorageFunction();
    ...
    for (FileStorageSubFunction inputSubFunction : getListSubFunctions()) {
      if (inputSubFunction.getSubFunctionType().equals(function)) {
        FileStorageOutput pdfOutput = inputSubFunction.executeSubFunction(pdfInput, outboundConnectorContext);
